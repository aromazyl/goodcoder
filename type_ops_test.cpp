...
TEST_F(TypeOpsTest, MakePairTest) {
  CompileAssert<GTR(IsSameType<GTR(MakePairs<int, int, bool, float>), Pair<int, Pair<int, Pair<bool, Pair<float, NullType>>>>>)>();
  CompileAssert<GTR(IsSameType<GTR(MakeLists<int, int, bool, float>), TypeList<int, TypeList<int, TypeList<bool, TypeList<float, NullType>>>>>)>();
}
